...
public class HederaListener {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("🚀 Starting Hedera Mirror Node Listener for contract: " + CONTRACT_ID);
    ...
    private static void pollMirrorNode() throws IOException {
        ...
        System.out.println("Polling Mirror Node at " + Instant.now());
        Request request = new Request.Builder().url(url).get().build();
        Response response = httpClient.newCall(request).execute();
        if (!response.isSuccessful()) {
            System.err.println("⚠️ Mirror Node request failed: " + response.code());
            return;
        }
        ...
        try {
            ...
            if (data.length() >= 64) {
                try {
                    ...
                    System.err.println("⚠️ Could not decode amount: " + e.getMessage());
                }
            }
            if (data.length() >= 128) {
                try {
                    ...
                    System.err.println("⚠️ Could not decode timestamp: " + e.getMessage());
                }
            }
            ...
            // Output
            System.out.println("✅ Proof payload:\n" + gson.toJson(payload));

            // POST to bridge endpoint
            sendToHLFBridge(payload.toString());
        } catch (Exception e) {
            System.err.println("⚠️ Error processing TokensLocked event:");
    ...
    private static void sendToHLFBridge(String json) throws IOException {
        ...
        try (Response response = httpClient.newCall(request).execute()) {
            System.out.println("🌉 Sent to HLF Bridge → Response Code: " + response.code());
        }
    }
