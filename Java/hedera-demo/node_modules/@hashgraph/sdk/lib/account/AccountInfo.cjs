...
class AccountInfo {
  ...
  static _fromProtobuf(info) {
    ...
    return new AccountInfo({
      ...
      hbarAllowances: (info.grantedCryptoAllowances != null ? info.grantedCryptoAllowances : []).map(allowance => _HbarAllowance.default._fromGrantedProtobuf(allowance)),
      tokenAllowances: (info.grantedTokenAllowances != null ? info.grantedTokenAllowances : []).map(allowance => _TokenAllowance.default._fromGrantedProtobuf(allowance)),
      nftAllowances: (info.grantedNftAllowances != null ? info.grantedNftAllowances : []).map(allowance => _TokenNftAllowance.default._fromGrantedProtobuf(allowance))
    });
  }
  /**
